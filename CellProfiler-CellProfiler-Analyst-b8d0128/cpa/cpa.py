...
class MainGUI(wx.Frame):
    ...
    def __init__(self, properties, parent, id=-1, **kwargs):
        ...
        tb.AddLabelTool(ID_ABHAKAR, 'abhakar', icons.boxplot.ConvertToBitmap(), shortHelp='Box Plot', longHelp='Launch Box Plot')
        tb.Realize()
...
#        hdlr.setFormatter(logging.Formatter('%(levelname)s | %(name)s | %(message)s [@ %(asctime)s in %(filename)s:%(lineno)d]'))
        ...
        self.Bind(wx.EVT_TOOL, self.launch_abhakar, id=ID_ABHAKAR)
    ...
    def launch_abhakar(self, evt = None):
        execfile('C:\\Users\\Dalitso\\Desktop\\CellProfiler-CellProfiler-Analyst-b8d0128\\cpa\\abhakar.py')
