...
class barControlPanel(wx.Panel):
    '''
    A panel with controls for selecting the source data for a barplot 
    '''
...
class barPanel(FigureCanvasWxAgg):
    '''
    Contains the guts for drawing bar plots.
    '''
    def __init__(self, parent, **kwargs):
        ...
        self.subplot2 = self.figure.add_subplot(111)
    ...
    def show_selection_in_table(self, evt=None):
        ...
        for i, sel in self.selection.items():
            ...
            if len(keys) > 0:
                ...
                grid = tableviewer.TableViewer(self, title='Selection from collection %d in bar'%(i))
    ...
    def redraw(self):        
        ...
        logging.debug('bar: Plotted %s points in %.3f seconds.'
    ...
    def set_x_scale(self, scale):
        ...
        if ignored > 0:
            logging.warn('bar masked out %s points with non-positive X values.'%(ignored))        
    ...
    def set_y_scale(self, scale):
        ...
        if ignored > 0:
            logging.warn('bar masked out %s points with non-positive Y values.'%(ignored))  
...
##            print 'barplot requires a properties file.  Exiting.'
