...
def HTS_regressionCoincidenceTest(dataDict,Type,printToFile,alpha):
    import numpy as np
    import math
    ...
    for iData in range(nDataSets):        
        ...
        for iGrp in range(nGroups):
            ...
            if Type ==  'crossplate':
                ...
                fit = np.polyfit(x[0],y[0],1)
                fit_fn = np.poly1d(fit)
                pyplot.plot(x[0],y[0], 'yo', x[0], fit_fn(x[0]), '--k')
                plots[iData].scatter (x[0],y[0])
                #pyplot.show()
